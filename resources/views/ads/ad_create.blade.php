                ...
                <div class="p-6 text-gray-900 mx-5 ">
                    <form method="post" action="{{ route('ad_create_post') }}" class="mt-6 space-y-6 "
                        ...
                        <select id="wheel" name="wheel_id"
                            ...
                            @foreach ($wheelModels as $wheelModel)
                                <option class="dark:text-gray-200 bg-white dark:bg-gray-800"
                                    {{ old('wheel_id') == $wheelModel->id ? 'selected' : '' }}
                                    value={{ $wheelModel->id }}>{{ $wheelModel->model }}</option>
                            @endforeach
                        </select>
                        @error('wheel_id')
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="photo" type="file" value="{{ old('photo[]') }}" multiple
